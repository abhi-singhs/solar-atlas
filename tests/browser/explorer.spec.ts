...
test('space music toggles from settings, the flight panel, and M, and persists', async ({ page }) => {
  ...
  page.on('pageerror', error => errors.push(error.message))
  await openApp(page)
  const atlas = page.locator('main.atlas')
  const dialog = page.getByRole('dialog', { name: 'Settings' })
  const music = dialog.getByRole('checkbox', { name: /^Space music/ })
  const volume = dialog.getByRole('slider', { name: 'Music volume' })
  ...
  await dialog.getByRole('button', { name: 'Close dialog' }).click()
  expect(await saved()).toMatchObject({ music: true, musicVolume: 0.4 })
  // Explore stays silent, so the audio context suspends a few seconds after music turns on, and again after a re-toggle.
  await expect(atlas).toHaveAttribute('data-music', 'stopped', { timeout: 15000 })
  await page.getByRole('button', { name: 'Settings' }).click()
  await music.uncheck()
  await music.check()
  await expect(atlas).toHaveAttribute('data-music', 'running')
  await dialog.getByRole('button', { name: 'Close dialog' }).click()
  await expect(atlas).toHaveAttribute('data-music', 'stopped', { timeout: 15000 })
  ...
  await page.getByRole('button', { name: 'Spaceship', exact: true }).click()
  await setFlightPanel(page, true)
  const button = page.getByRole('region', { name: 'Spacecraft controls' }).getByRole('button', { name: 'Music', exact: true })
  ...
  await expect(atlas).toHaveAttribute('data-music', 'stopped', { timeout: 15000 })
  expect(await saved()).toMatchObject({ music: false, musicVolume: 0.4 })
  await button.click()
  await expect(button).toHaveAttribute('aria-pressed', 'true')
  await expect(atlas).toHaveAttribute('data-music', 'running')
